...
void Filter::Update(void)
{
    ...
    //Catch Circular Update
    if (updating)
    {
        sprintf(msg, "%s: circular dependance", this->FilterName());
...
void Shrinker::Execute(void)
{
    ...
    for (i = 0; i < new_h; i++)
    {
        for (j = 0; j < new_w; j++)
        {  
            red = img1->getPixel(2  * i * old_w + j * 2).r; 
            green = img1->getPixel(2  * i * old_w + j * 2).g; 
            blue = img1->getPixel(2  * i * old_w + j * 2).b; 
